...
function Premium() {
    ...
    return (
        <div className="bg-black min-h-screen py-10 px-6" id="premium-collection ">
            <h1 class="text-5xl md:text-7xl font-extrabold text-center tracking-tight p-10 mt-10" >
                <span class="bg-gradient-to-r from-white via-gray-400 to-gray-600 bg-clip-text text-transparent drop-shadow-lg ">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8 ">
                {premium.map((product) => (
                    ...
                    >                    <Link key={product.id} to={`/product/${product.id}`} className='block'> 
                        ...
                        <p className={product.stoke > 0 ? "text-green-700 mt-1" : "text-red-500 font-bold mt-1"
                        }>{product.stoke > 0 ? `Stock left : ${product.stoke}` : 'Out Of Stock'}</p>
                                           </Link>
